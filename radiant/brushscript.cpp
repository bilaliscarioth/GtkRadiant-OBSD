...
const char* GetParam( char*& pBuffer ){
	...
	while ( *pBuffer != (char)0x0 && isspace( *pBuffer ) ) // skip and whitespace
		pBuffer++;
	...
	if ( bStringMode ) {
		while ( *pBuffer != (char)0x0 && *pBuffer != '\"' )
			strParam += *pBuffer++;
	}
	else
	{
		while ( *pBuffer != (char)0x0 && *pBuffer != ' ' && *pBuffer != ')' && *pBuffer != ',' )
			strParam += *pBuffer++;
	}

	if ( *pBuffer != (char)0x0) { // skip last char
		pBuffer++;
	}
...
void RunScript( char* pBuffer ){
	...
	{
		while ( *pBuffer != (char)0x0 && *pBuffer != '_' )
			pBuffer++;

		char* pTemp = pBuffer;
		int nLen = 0;
		while ( *pTemp != (char)0x0 && *pTemp != '(' )
		{
			pTemp++;
			nLen++;
		}
...
#else
				if ( strncmp( g_ScriptCmds[i].m_pName, pBuffer, nLen ) ==  0 ) {
#endif
